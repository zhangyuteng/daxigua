...
- 🎯 Entry points to execute your program using `python -m <daxigua>` or `$ daxigua` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/zhangyuteng/daxigua/branch/main/graph/badge.svg?token=daxigua_token_here)](https://codecov.io/gh/zhangyuteng/daxigua)
[![CI](https://github.com/zhangyuteng/daxigua/actions/workflows/main.yml/badge.svg)](https://github.com/zhangyuteng/daxigua/actions/workflows/main.yml)

Awesome daxigua created by zhangyuteng

## Install

from source
```bash
git clone https://github.com/zhangyuteng/daxigua daxigua
cd daxigua
make install
```
...
```bash
$ daxigua run --port 8080
```

or

```bash
python -m daxigua run --port 8080
```
...
```bash
❯ daxigua --help
Usage: daxigua [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ daxigua create-user --help
Usage: daxigua create-user [OPTIONS] USERNAME PASSWORD
...
```bash
daxigua create-user admin admin --superuser
```
...
Run with `daxigua run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `daxigua create-user`.
...
-----------------------------------------------------
daxigua/__init__.py              4      0   100%
daxigua/app.py                  16      1    94%
daxigua/cli.py                  21      0   100%
daxigua/config.py                5      0   100%
daxigua/db.py                   10      0   100%
daxigua/models/__init__.py       0      0   100%
daxigua/models/content.py       47      1    98%
daxigua/routes/__init__.py      11      0   100%
daxigua/routes/content.py       52     25    52%
daxigua/routes/security.py      15      1    93%
daxigua/routes/user.py          52     26    50%
daxigua/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in daxigua/default.toml.
